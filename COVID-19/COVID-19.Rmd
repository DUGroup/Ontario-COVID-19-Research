...
The map provides data only for those cases that have an identified health unit. Currently there are `r n_pending` out of the `r length (df$Case_number)` cases that are listed as Pending or Not Available. 

The number inside the bubble shows the number of Public Health Units in that area that have reported cases of COVID-19. Clicking the bubble will zoom into that level of geography. 
...
leaflet(data = phu) %>%
  ...
  addCircleMarkers(~long, ~lat, 
                   radius = ~sqrt(n) + 3,
                   label = ~as.character(paste0(Public_Health_Unit, " ", n, " cases")),
                   popup = ~as.character(paste0(Public_Health_Unit, " ", n, " cases")), 
                   stroke = FALSE, 
                   fillOpacity = 0.8,
                   color = "purple",
